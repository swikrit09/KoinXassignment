...
function TrendingCoins() {
    const [coinsData, setCoinsData] = useState([]); // Define the type for coinsData
...
function Coin({ symbol, name, img, changePercentage }) {
    ...
    return (
        <div className="flex my-2 justify-between text-center py-1">
            ...
            <div
                className={`flex items-center justify-center rounded-lg p-2 h-8 ml-10 ${isPositiveChange ? "bg-green-300/20" : "bg-red-300/20"
                    }`}
            >
                <svg
                    viewBox="0 0 100 100"
                    className={`w-4 fill-current ${isPositiveChange ? "text-green-600" : "text-red-600"
                        }`}
                ...
                <span
                    className={`ml-2 text-sm font-bold ${isPositiveChange ? "text-green-600" : "text-red-600"
                        }`}
                >
                    {changePercentage}%
                </span>
